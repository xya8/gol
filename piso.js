module.exports = class Piso {
    ...
    for (const newCell of cells) {
      if (newCell) {
        ...
        for (let i in grassEaterEaterArr) {
          if (
            grassEaterEaterArr[i].x == newCell[0] &&
            grassEaterEaterArr[i].y == newCell[1]
          ) {
  ...
  move(x, y) {
    // let getSkinArr = this.getSkin(this.x, this.y);
    // console.log(getSkinArr);
    // for (const coord of getSkinArr) {
    //   console.log(coord, "cooooord");
